        ...
        rom_groups = {
            "A Ressha de Ikou 4 - Evolution": [
                (
                    files[0],
                    "japan",
                    "A Ressha de Ikou 4 - Evolution (Japan) (Gentei Set).zip",
                ),
                (
                    files[1],
                    "japan",
                    "A Ressha de Ikou 4 - Evolution (Japan) (Rev 1).zip",
                ),
...
def test_cross_regional_removal():
    ...
    try:
        ...
        assert (
            "Biohazard" in removed_file.name
        ), f"Should remove Japanese version, got: {removed_file.name}"
        # The file path should be the Japanese file
        assert (
            removed_file.name == "Biohazard (Japan).zip"
        ), f"Should remove Japanese file, got: {removed_file.name}"
...
def test_version_info_extraction():
    ...
    for filename, expected in test_cases:
        ...
        assert expected in version_info or (
            not expected and not version_info
