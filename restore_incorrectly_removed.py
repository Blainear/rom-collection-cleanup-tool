...
def analyze_removed_files(removed_folder: Path) -> Dict[str, List[Path]]:
    ...
    # Get all ROM files
    rom_extensions = {'.zip', '.bin', '.cue', '.iso', '.chd', '.nes', '.snes', '.gb', '.gba', '.nds', '.md', '.gen'}
    rom_files = []
...
def identify_files_to_restore(
    removed_groups: Dict[str, List[Path]],
    main_directory: Path
) -> List[Path]:
    ...
    for base_name, removed_files in removed_groups.items():
        ...
        for file_path in main_directory.rglob("*"):
            if (file_path.is_file() and 
                get_base_name(file_path.name) == base_name and
                file_path.parent.name not in ["removed_duplicates", "to_delete"]):
        ...
        for removed_file in removed_files:
            ...
            if any(keyword in version_info.lower() for keyword in 
                   ['rev 1', 'revision 1', 'special', 'limited', 'premium', 'deluxe']):
...
def restore_files(files_to_restore: List[Path], 
                 main_directory: Path, 
    ...
    for file_path in files_to_restore:
        ...
        logger.info(f"  {'WOULD RESTORE' if dry_run else 'RESTORING'}: {file_path.name}")
